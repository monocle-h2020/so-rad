...
def main(conf):
    ...
    print("read data for {0} second".format(test_duration_single_reads))
    # get protocol from config
    t1 = time.time()
    while time.time() < t1 + test_duration_single_reads:

        u, t, p, r = tpr_manager.update_pitch_roll_single()
    ...
    while time.time() < t1 + test_duration_monitor_average:
        if None in [tpr_manager.avg_updated, tpr_manager.tilt_avg, tpr_manager.tilt_std,
            ...
            print("Waiting for average. Buffer length: {0}, oldest record: {1}".format(len(tpr_manager.buffer_tilt), tpr_manager.buffer_time[0]))
        else:
            print("{0} | Tilt: {1:2.2f} ({2:2.2f} \t Pitch: {3:2.2f} ({4:2.2f}) \t Roll: {5:2.2f} ({6:2.2f})"\
