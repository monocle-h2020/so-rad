...
if __name__ == '__main__':
    ...
    if motor_deg_pos != -motor['home_pos']:
        ...
        moving, motor_step_pos = motor_func.motor_moving(motor['serial'], target_pos, tolerance=10)
        motor_deg_pos = float(motor_step_pos) / motor['steps_per_degree']

        print(f"Homing motor.. {motor_deg_pos} deg ({motor_step_pos}) --> {-motor['home_pos']} deg ({target_pos})")
        motor_func.rotate_motor(motor_commands_dict, -motor['home_pos'], motor['serial'])

        moving = True
        while moving and (time.perf_counter()-t0<5):
            moving, motor_step_pos = motor_func.motor_moving(motor['serial'], target_pos, tolerance=300)
            motor_deg_pos = float(motor_step_pos) / motor['steps_per_degree']
            if moving is None:
        ...
        print("Motor in home position (corrected for offset)")
        moving, motor_step_pos = motor_func.motor_moving(motor['serial'], -1.*motor['home_pos'], tolerance=300)
        motor_deg_pos = float(motor_step_pos) / motor['steps_per_degree']
